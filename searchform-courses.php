 ...
 */
?>
<form role="search" method="get" class="search-form" id="courses-search" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	<label class="hidden assistive-text screen-reader-text" for="courses-search"><?php echo _e( 'Search courses:', 'label' ); ?>
	</label>
	<input type="search" class="search-field" placeholder="<?php echo esc_attr_e( 'Search all courses &hellip;', 'placeholder' ); ?>" value="<?php echo get_search_query(); ?>" name="s" id="courses-search" />
