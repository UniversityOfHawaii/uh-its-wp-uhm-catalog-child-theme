...
class mwm_aal {
	...
	var $findh = true;
	var $findAnchor = true;
	var $tag = "anchor-display";
	var $htmltag = "<!--anchor_display-->";
	var $isTagUsed = false;
	...
	function add_anchors_to_content($content){
		if(count($this->links) >= 1){
			foreach ($this->links as $val) {
				$rtext='<a class="sub-section" id="'.urlencode($this->toAscii(strip_tags($val[2]))).'"></a>';
	...
	function output_sidebar_links(){
		...
		if ((count($this->links) >= 1) && (is_single())){
			$info = '<div class="anchors-sidebar-container">';
				foreach ($this->links as $val) {
				$urlval = urlencode($this->toAscii(strip_tags($val[2])));
					$info.='<li><a href="#'.$urlval.'">'.strip_tags($val[2]).'</a></li>';
				}
